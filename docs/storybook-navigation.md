...
The project switcher, command palette, action palette, and file quick-open use the public SDK [search palette](sdk-search-palette.md). The component catalog covers that dialog and the SDK's nonmodal listbox for inline completion. File Viewer destinations and application-wide shortcut orchestration are unchanged.
...
The browser test requires freshly built catalogs and the workspace Playwright Chromium installation. It runs the modal navigation/control stories twice in the same document, checks declared diagnostics exactly, rejects unexpected warnings and errors, and verifies persisted fixture storage resets. It is opt-in because ordinary unit tests do not build static catalogs. Production prompt-completion tests and SDK inline-listbox browser tests run in the ordinary renderer/SDK suites.

The screenshot manifest selects 37 navigation/control cases across light and dark themes, desktop and narrow page sizes, and component-sized viewports. Use the canonical Linux commands in [the visual review guide](storybook-visuals.md), not native screenshots, to update approvals. Coverage remains incremental as described in [the coverage guide](storybook-coverage.md).
...
- Shared controls include selected, filtered, empty, loading, narrow, overflowing, and repeated keyboard-selection states inside the SDK palette modal. `Components/Inline palette controls` owns the reusable nonmodal listbox states; `Components/Prompt input` retains production prompt-completion coverage.
...
Separate production fixes are tracked as KVG-5001 for stale file-search responses, KVG-4800 for command-palette loading after dismissal, and KVG-4811 for reduced-motion support in the empty-board illustration. This slice does not change those production workflows.
