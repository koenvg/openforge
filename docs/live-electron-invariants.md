...
Pull-request CI runs `first-attachment` and `detach-during-recovery` in one boot through the `Live Electron Terminal Invariants` job in `.github/workflows/ci.yml`. The job uses the `macos-15` ARM runner with Rust, Zig/Ghostty, Bun, Node, and pnpm. It uploads the complete invariant artifact root on success or failure with seven-day retention.
