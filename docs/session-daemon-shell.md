...
Protocol v5 frames encode terminal output, recovery and input bytes as base64 text instead of JSON number arrays. A 512 KiB output batch drops from about 1.9 MB to about 700 KB. Persisted checkpoints keep number arrays, and readers accept both forms. v5 peers reject older frames, and the Session Client reports an older daemon as an unsupported protocol version.
