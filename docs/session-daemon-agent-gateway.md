...
- A headless repo-aware review generation has no PTY allocation, so the Sidecar issues it a generation-scoped credential of the same shape in its own mode-0700 directory under the application data directory. The Sidecar ingress accepts that bearer for the CLI transport allowlist, rejects caller-supplied ownership headers, and rejects any bearer that is neither the controller's nor a live generation's. The token is revoked and the file removed when the generation ends. The directory is swept on startup, since no generation outlives the process.
