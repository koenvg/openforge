...
Desktop revision: KVG-3092 worktree
...
| Narrow authenticated routes | Rust Companion Gateway Create/Start/Delete/Complete, visibility, validation, ownership, body-rejection, and no-generic-invoke tests | Pass |
...
| Foreground recovery | Dart tests cover reconnect, selected-Project invalidations, stream gaps, gateway shutdown, authorization loss, credential failure, certificate mismatch, incompatible protocol, stale Board clearing, and uncertain Create outcomes with check-before-retry guidance | Pass |
...
- `pnpm mobile:contract:check` — pass, 12 generated Companion operations compatible.
- `pnpm exec tsc --noEmit` and `pnpm lint` — pass.
- `pnpm test` — pass, 354 files and 3,820 tests.
- `cd src-tauri && cargo test` — pass, 1,388 unit tests plus 3 sidecar contract tests.
- `./scripts/mobile-companion check` — pass; formatting clean, analyzer reports no issues, and 210 Flutter tests pass.
- `./scripts/mobile-companion build-android` — pass; debug APK copied to `$HOME/OpenForge-Companion-debug.apk` with SHA-256 `464bad8d1086e9b154c6bbf90d501321b0e2a856b8c5d6d82a22873d617e3b09`.
