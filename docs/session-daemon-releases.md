...
`.github/workflows/packaged-session-runtime.yml` runs independently of the broad CI gate, requires native host/Node architectures, rejects Rosetta, and uploads evidence even on failure. Its arm64 job uses the existing `macos-14` packaging baseline; Intel uses `macos-15-intel`. The first arm64 attempt on `macos-15` failed in Whisper/ggml compilation before daemon validation (`vmmlaq_s32` requires `i8mm`); KVG-5152 tracks that separate toolchain issue.
