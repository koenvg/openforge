...
KVG-5235 checked the current bundled-plugin sources against the host and Storybook stylesheets. Storybook's `plugins/*/src` wildcard did not reliably cover GitHub Sync; the host and Storybook now share an explicit, paint-only `@source inline` list in `src/app.css`. Task Browser adds `bg-of-danger/10`, `bg-of-surface-subtle/60`, and `border-of-danger/30`; Terminal's `text-of-text/50` was already in the list. File Viewer has no `of-*` paint utilities yet, and Task Schedules uses scoped token CSS. The Storybook-specific File Viewer and Task Schedules sources remain as before. Task Browser's review and error views also need six Storybook-only layout classes; `storybook/shared/preview.css` lists only those six rather than scanning the whole plugin. This keeps its scrollable review panel within the same 288px height cap as the host. Do not add whole-plugin scans to fix future missing paint: they can enable unrelated layout rules and change card bounds. KVG-4869 should add any new File Viewer semantic classes to the inline list; KVG-4874 should preserve this contract when removing daisyUI. `scripts/bundled-plugin-source-discovery.test.mjs` checks these consumers and an isolated build with automatic discovery disabled.
