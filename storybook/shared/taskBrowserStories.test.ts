...
describe('Task Browser catalog', () => {
  it('resets resources and repeats public page and component interactions without diagnostics', async () => {
    ...
    try {
      ...
      await components.ReviewOverflow.run({ canvasElement: canvas, testingLibraryRender: render })
      expect(await view.findByText('1 screenshot · 4 annotations')).toBeTruthy()
      expect(document.body.dataset.taskBrowserReady).toBe('components-task-browser--review-overflow')
