...
{#snippet treeGuides(depth: number)}
  {#each Array(depth) as _, guideDepth}
    <span
      class="pointer-events-none absolute inset-y-0 w-px bg-of-border/80"
  ...
  {#if depth > 0}
    <span
      class="pointer-events-none absolute h-px w-2 bg-of-border/80"
...
{#if node.isDir}
  <button
    class="relative flex min-h-[var(--of-control-height)] w-full cursor-pointer items-center gap-2 px-3 text-[13px] font-medium text-of-text/75 transition-colors hover:bg-of-surface-subtle/70"
  ...
  >
    ...
    {#if expanded}
      <ChevronDown size={14} strokeWidth={2} class="shrink-0 text-of-text/45" aria-hidden="true" />
    {:else}
      <ChevronRight size={14} strokeWidth={2} class="shrink-0 text-of-text/45" aria-hidden="true" />
    {/if}
    ...
    <span class="flex min-w-0 flex-1 items-center overflow-hidden whitespace-nowrap text-left" title={node.fullPath}>
      {#each node.name.split('/') as segment, index}
        {#if index > 0}<span class="px-1 text-of-text/30" aria-hidden="true">/</span>{/if}
...
{:else if node.file}
  ...
  <div
    class="relative flex min-h-10 w-full items-center gap-1 pr-2 transition-colors {selected ? 'bg-of-accent/8 border-l-2 border-l-of-accent' : 'hover:bg-of-surface-subtle/70'} {active ? 'group-focus-within/tree:ring-2 group-focus-within/tree:ring-of-accent group-focus-within/tree:ring-inset' : ''}"
  ...
  >
    ...
    {/if}
    <button
      class="flex-1 min-w-0 flex items-center gap-2 text-[13px] transition-colors py-2 text-of-text text-left focus:outline-none"
      role="treeitem"
      aria-level={depth + 1}
      tabindex="-1"
    ...
    >
      ...
      <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left text-[13px] {reviewed ? 'line-through text-of-text/50' : ''}" aria-label={reviewed ? `Reviewed file ${node.file.filename}` : undefined} title={node.file.filename}>{node.name}</span>
      <span
      ...
      <span class="flex shrink-0 items-center gap-1.5 text-[13px] tabular-nums" aria-label="{node.file.additions} additions and {node.file.deletions} deletions">
        {#if node.file.additions > 0}<span class="font-medium text-of-success">+{node.file.additions}</span>{/if}
        {#if node.file.deletions > 0}<span class="font-medium text-of-danger">−{node.file.deletions}</span>{/if}
      </span>
    </button>
  </div>
{/if}
