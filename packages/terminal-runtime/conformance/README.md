...
`TerminalView.capturePresentation()` supplies deterministic, renderer-neutral rows and cells. Text, cell widths, styles, buffer choice, and selection are asserted as data. `TerminalView.drainPresentation()` resolves only after queued writes parse and a later renderer frame fires. Throughput, first interaction, recovery, and screenshot timings all stop at that drain evidence rather than at `writeLive()` enqueue.

## Visual bounds

The checked-in matrix has twelve screenshots: six presentation recordings for the Agent Terminal at dark/DPR 1, and the same six for the plugin shell at light/DPR 2. `pixelmatch` uses a per-pixel threshold of `0.15`; at most `1%` of pixels may differ. A failure writes the actual image and a diff image to the artifact directory.
