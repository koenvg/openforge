...
Task context menus must use `TaskContextMenu` (`src/components/shared/tasks/TaskContextMenu.svelte`) which provides Start Task, Move to Focus/Low-Fire, and `Complete 🏁`. `Complete` is the single terminal action: it is a confirmed, destructive operation that permanently deletes the task along with its worktree and branch (there is no Done/reopen flow) — never wire it up as a non-confirmed or non-destructive action. For non-task context menus, use the lower-level `ContextMenu` + `ContextMenuItem` primitives — never build inline context menu markup.
