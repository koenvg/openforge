...
Vim navigation uses `useVimNavigation` composable from `src/lib/useVimNavigation.svelte.ts` for j/k/G/gg/Enter/Escape/q/x/h/l. All plain-key vim bindings must check `isInputFocused()` from `src/lib/domUtils.ts`. `g`-prefix view navigation (gb/gp/gs/gc/gw/g,) handled in App.svelte via `src/lib/vimGoto.ts`. Visual focus uses `ring-2 ring-primary rounded` (daisyUI semantic, no hex).
AI provider logic uses a registry pattern (`src/lib/providers.ts`). Never branch on provider ID strings (e.g. `if provider === 'claude-code'`) in components — use `getProvider(id)` and check capabilities (`supportsPermissionMode`, `supportsAgentSelection`, `loadSessionLogs`, `loadAgents`). To add a new provider: register it in `providers.ts`, create its agent panel component, and add it to the `panelComponents` map in `AgentPanel.svelte`.
