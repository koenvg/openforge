...
{#if controller.showDiscardConfirmation}
  <Modal ariaLabel="Discard schedule changes" maxWidth="420px" initialFocus="#discard-schedule-changes" onClose={controller.cancelDiscard}>
...
{#if controller.schedulePendingDelete}
  <Modal ariaLabel="Delete schedule confirmation" maxWidth="420px" initialFocus="#confirm-delete-schedule" closeDisabled={controller.deleting} onClose={controller.cancelDelete}>
    ...
    <div class="mt-5 flex justify-end gap-2">
      ...
      <button id="confirm-delete-schedule" class="btn btn-error min-h-10" type="button" disabled={controller.deleting} onclick={() => { void controller.deleteSchedule() }}>{controller.deleting ? 'Deleting…' : 'Delete schedule'}</button>
    </div>
  </Modal>
{/if}
