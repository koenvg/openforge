...
export async function acquire(taskId: string): Promise<PoolEntry> {
  ...
  entry.unlisteners.push(getPluginContext().onEvent(`pty-output-${taskId}`, (payload) => {
    const event = { payload: payload as PtyEvent }
  ...
  entry.unlisteners.push(getPluginContext().onEvent(`pty-exit-${taskId}`, (payload) => {
    const event = { payload: payload as PtyEvent }
    const instanceId = event.payload.instance_id
    if (instanceId != null && entry.currentPtyInstance != null && instanceId !== entry.currentPtyInstance) {
      return
