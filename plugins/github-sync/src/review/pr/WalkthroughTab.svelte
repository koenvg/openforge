...
<div class="flex flex-col h-full min-h-0 overflow-hidden">
  {#if lifecycle.view === 'loading'}
    <div class="flex flex-col items-center justify-center flex-1 gap-3 text-of-text/50 text-sm" role="status">
      <LoadingIndicator size="md" decorative class="text-of-accent" />
      <span>Loading walkthrough…</span>
    </div>
  {:else if lifecycle.view === 'loadError'}
    <div class="flex flex-col items-center justify-center flex-1 gap-3 text-of-danger text-sm text-center p-5" role="alert">
  ...
  {:else if lifecycle.view === 'absent'}
    <div class="flex flex-col items-center justify-center flex-1 gap-4 text-center p-8 max-w-xl mx-auto">
      <h3 class="text-lg font-semibold text-of-text m-0">Walk me through this PR</h3>
      <p class="text-sm text-of-text/60 m-0">
  ...
  {:else if lifecycle.view === 'generating'}
    <div class="flex flex-col items-center justify-center flex-1 gap-3 text-of-text/60 text-sm" role="status">
      <LoadingIndicator size="md" decorative class="text-of-accent" />
  ...
  {:else if lifecycle.view === 'no-submissions'}
    <div class="flex flex-col items-center justify-center flex-1 gap-3 text-of-text/70 text-sm text-center p-5">
  ...
  {:else if lifecycle.view === 'unaligned'}
    <div class="flex flex-col items-center justify-center flex-1 gap-3 text-of-text/60 text-sm text-center p-5">
  ...
  {:else}
    {#if lifecycle.view === 'provisional'}
      <div class="flex items-center justify-between gap-3 px-4 py-2 bg-of-info/10 border-b border-of-info/30 text-xs" role="status" aria-live="polite" aria-atomic="true">
        <span>
    ...
    {/if}
    {#if stale}
      <div class="flex items-center justify-between gap-3 px-4 py-2 bg-of-warning/10 border-b border-of-warning/30 text-xs">
        <span class="text-of-on-warning/80">
    ...
    <div class="flex items-start gap-2 px-4 {stepDetailsExpanded ? 'py-2.5' : 'py-1'} border-b border-of-border shrink-0">
      <div class="flex flex-col gap-1.5 min-w-0 flex-1">
        <div class="flex items-baseline gap-2 min-w-0">
          <span class="text-[11px] font-semibold uppercase tracking-wider text-of-accent tabular-nums shrink-0">Step {activeStepIndex + 1}</span>
          <span class="text-[10px] font-medium uppercase tracking-wider text-of-text/40 shrink-0">of {totalSteps}</span>
          <h3 class="text-sm font-semibold text-of-text m-0 leading-snug min-w-0 {stepDetailsExpanded ? '' : 'truncate'}">{stepTitle}</h3>
        </div>
        ...
        <div class="flex flex-col gap-2 max-h-[28vh] overflow-y-auto pr-1 {stepDetailsExpanded ? '' : 'hidden'}">
          {#if stepDetailsExpanded && stepSummary}
            <p class="text-sm leading-relaxed text-of-text/80 m-0">{stepSummary}</p>
          {/if}
          {#if lifecycle.ready}
