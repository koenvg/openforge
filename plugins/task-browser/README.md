...
Task Browser visual feedback stays on the live page. Numbered region markers share the page with a collapsible lower-right comment card for unsent annotations on the exact visible URL. The card follows the configured OpenForge light or dark appearance and updates without resetting its disclosure state. It leaves rendering, hit testing, accessibility, and screenshot capture while region selection or viewport capture is active.

Each card row has a labelled keyboard-operable delete control. A trusted activation requests deletion from the Task Browser plugin; Electron validates the current surface generation, exact URL, and synchronized annotation number, but does not remove the row or marker itself. The plugin applies the request through the same draft persistence, capture cleanup, undo, retry, and live synchronization workflow used by its review controls. Programmatic page activations are ignored, and the row remains until plugin synchronization updates the host-owned overlay.

Use the plugin's compact review controls to edit comments or normalized marker geometry, remove findings and captures, undo the latest change, recover persistence failures, and review findings from other URLs. Unsent drafts persist per Task; save failures remain retryable in memory, and missing background PNGs are reported on each affected finding. **Send to agent** submits the corrected deterministic Markdown report immediately while retained background PNGs remain immutable Task artifacts for the Agent.
