...
///
/// # Arguments
/// * `diff_output` - Raw unified diff string (e.g., from `git diff`)
///
/// # Returns
/// Vec of TaskFileDiff, one per file. Empty vec if input is empty or no diffs found.
pub fn parse_unified_diff(diff_output: &str) -> Vec<TaskFileDiff> {
