...
/// Scan an agents directory (e.g. `.opencode/agents/`) for `.md` files.
/// Each `.md` file is an agent definition with name, hidden, and mode in YAML frontmatter.
/// Returns a Vec of AgentInfo with parsed metadata.
/// Missing fields (hidden, mode) return None.
pub fn scan_agents_directory(dir: &Path) -> Vec<crate::opencode_client::AgentInfo> {
    ...
    for entry in entries.flatten() {
        ...
        let (fm_name, fm_hidden, fm_mode) = parse_agent_frontmatter(&content);
        let name = fm_name.unwrap_or(file_stem);
...
/// Parse agent frontmatter to extract name, hidden, and mode.
/// Frontmatter is YAML between `---` delimiters at the start of the file.
fn parse_agent_frontmatter(content: &str) -> (Option<String>, Option<bool>, Option<String>) {
    let trimmed = content.trim_start();
    if !trimmed.starts_with("---") {
    ...
    for line in frontmatter.lines() {
        ...
        if trimmed_line.starts_with("name:") {
            name = Some(trimmed_line.trim_start_matches("name:").trim().to_string());
        } else if trimmed_line.starts_with("hidden:") {
            let val = trimmed_line.trim_start_matches("hidden:").trim();
            hidden = Some(val == "true");
        } else if trimmed_line.starts_with("mode:") {
            mode = Some(trimmed_line.trim_start_matches("mode:").trim().to_string());
        }
    }
...
mod tests {
  ...
  "enabledPlugins": {
    ...
    #[test]
    fn test_scan_agents_directory_happy_path() {
        let dir = tempfile::tempdir().unwrap();

        // File 1: agent with name, hidden, and mode in frontmatter
        let file1 = dir.path().join("oracle.md");
        std::fs::write(
            &file1,
            "---\nname: oracle\nhidden: false\nmode: chat\n---\n# Oracle Agent",
        )
        ...
        // File 2: agent with name only (hidden and mode missing)
        let file2 = dir.path().join("researcher.md");
        std::fs::write(&file2, "---\nname: researcher\n---\n# Researcher Agent").unwrap();

        let mut agents = scan_agents_directory(dir.path());
        agents.sort_by(|a, b| a.name.cmp(&b.name));

        assert_eq!(agents.len(), 2);

        // "oracle" with hidden=false and mode="chat"
        assert_eq!(agents[0].name, "oracle");
        assert_eq!(agents[0].hidden, Some(false));
        assert_eq!(agents[0].mode, Some("chat".to_string()));

        // "researcher" with hidden=None and mode=None (missing fields)
        assert_eq!(agents[1].name, "researcher");
        assert_eq!(agents[1].hidden, None);
        assert_eq!(agents[1].mode, None);
    }

    #[test]
    fn test_scan_agents_directory_nonexistent() {
        let result = scan_agents_directory(Path::new("/nonexistent/path/that/does/not/exist"));
        assert!(result.is_empty());
    }

    #[test]
    fn test_scan_agents_directory_empty_dir() {
        let dir = tempfile::tempdir().unwrap();
        let result = scan_agents_directory(dir.path());
        assert!(result.is_empty());
    }

    #[test]
    fn test_scan_agents_directory_ignores_non_md_files() {
        let dir = tempfile::tempdir().unwrap();

        // Create a .txt file (should be ignored)
        let text_file = dir.path().join("agent.txt");
        std::fs::write(&text_file, "---\nname: should-ignore\n---\n# Body").unwrap();

        // Create a .md file (should be included)
        let md_file = dir.path().join("oracle.md");
        std::fs::write(&md_file, "---\nname: oracle\n---\n# Body").unwrap();

        let agents = scan_agents_directory(dir.path());
        assert_eq!(agents.len(), 1);
        assert_eq!(agents[0].name, "oracle");
    }

    #[test]
    fn test_scan_agents_directory_name_fallback() {
        let dir = tempfile::tempdir().unwrap();

        // File with no name in frontmatter — should use filename stem
        let file = dir.path().join("expert.md");
        std::fs::write(&file, "---\nhidden: true\n---\n# Body").unwrap();

        let agents = scan_agents_directory(dir.path());
        assert_eq!(agents.len(), 1);
        assert_eq!(agents[0].name, "expert");
        assert_eq!(agents[0].hidden, Some(true));
        assert_eq!(agents[0].mode, None);
    }

    #[test]
    fn test_scan_agents_directory_hidden_true() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let agents = scan_agents_directory(dir.path());
        assert_eq!(agents.len(), 1);
        assert_eq!(agents[0].name, "secret");
        assert_eq!(agents[0].hidden, Some(true));
    }

    #[test]
    fn test_scan_agents_directory_multiple_agents() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let file3 = dir.path().join("agent3.md");
        std::fs::write(&file3, "---\nname: third\n---\n# Body").unwrap();
        ...
        assert_eq!(agents[0].hidden, Some(false));
        assert_eq!(agents[0].mode, Some("code".to_string()));

        assert_eq!(agents[1].name, "second");
        assert_eq!(agents[1].hidden, Some(true));
        assert_eq!(agents[1].mode, Some("chat".to_string()));

        assert_eq!(agents[2].name, "third");
        assert_eq!(agents[2].hidden, None);
        assert_eq!(agents[2].mode, None);
    }
}
