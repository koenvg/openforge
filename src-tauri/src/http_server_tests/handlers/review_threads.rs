...
#[tokio::test]
async fn test_a_headless_generation_credential_writes_a_thread_a_later_listing_returns() {
    let (mut state, temp_dir) = test_state("http_review_threads_generation_identity");
    let mut pty_manager = crate::pty_manager::PtyManager::new();
    pty_manager.enable_daemon_shell(
        temp_dir.path().into(),
        "/unused-daemon".into(),
    ...
    let as_generation = |mut request: Request<Body>, token: &str| {
        request.headers_mut().insert(
            "authorization",
    ...
    let created = router
        .clone()
        .oneshot(as_generation(
            post("/review_threads/create", create_body()),
            credential.token(),
        ))
        .await
        .expect("create response");
    assert_eq!(created.status(), StatusCode::OK);
    let thread_id = response_body_json(created).await["id"]
        .as_str()
        .expect("thread id")
        .to_string();
    ...
    let listed = response_body_json(
        router
            .clone()
            .oneshot(as_controller(post("/review_threads/list", scope_body())))
            .await
            .expect("list response"),
    )
    .await;
    ...
    let refused = router
        ...
        .await
        .expect("post-generation response");
