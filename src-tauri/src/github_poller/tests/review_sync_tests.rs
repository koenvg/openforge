...
fn make_authored_search_pr(id: i64) -> SearchPrResult {
    SearchPrResult {
        id,
        number: id,
        ...
        user_avatar_url: Some("https://avatars.example/octocat".to_string()),
        repo_owner: "acme".to_string(),
...
fn make_authored_details(id: i64) -> PullRequest {
    PullRequest {
        number: id,
        title: format!("Authored PR {id}"),
        state: "open".to_string(),
...
fn make_authored_enrichment(id: i64) -> EnrichedAuthoredPr {
    let pr = make_authored_search_pr(id);
    ...
    let combined_status = CombinedStatusResponse {
    ...
    let details = make_authored_details(id);

    aggregate_authored_pr_enrichment(pr, check_runs, combined_status, reviews, details)
}

fn make_authored_pr_for_persistence(id: i64) -> EnrichedAuthoredPr {
    EnrichedAuthoredPr {
        pr: make_authored_search_pr(id),
        created_at: 1704067200,
        ci_status: Some("success".to_string()),
        ci_check_runs: Some("[]".to_string()),
...
#[test]
fn authored_pr_enrichment_aggregates_github_responses() {
    let enriched = make_authored_enrichment(71);

    assert_eq!(enriched.created_at, 1704067200);
    ...
    assert_eq!(
        ...
        serde_json::json!([{
            ...
            "status": "completed",
            "conclusion": "success",
            "html_url": "https://github.com/acme/repo/actions/runs/71"
        }])
    );
}

#[test]
fn authored_pr_persistence_upserts_deletes_stale_rows_and_publishes_event() {
    let (db, _temp_dir) = make_test_db("authored_pr_persistence_seam");
    let (event_tx, mut event_rx) = tokio::sync::broadcast::channel(4);
    let events = GitHubEventTarget::sidecar(Some(event_tx));

    persist_authored_prs(&db, &events, &[make_authored_pr_for_persistence(71)], &[71])
        .unwrap_or_else(|error| panic!("persist initial authored PR: {error}"));
    let _ = event_rx.try_recv().expect("initial authored PR event");

    persist_authored_prs(&db, &events, &[make_authored_pr_for_persistence(72)], &[72])
        .unwrap_or_else(|error| panic!("persist replacement authored PR: {error}"));

    let rows = db.get_all_authored_prs().expect("read authored PRs");
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].id, 72);
    assert_eq!(rows[0].ci_status.as_deref(), Some("success"));
    assert_eq!(rows[0].review_status.as_deref(), Some("approved"));
    assert_eq!(rows[0].mergeable, Some(true));
    assert_eq!(rows[0].mergeable_state.as_deref(), Some("clean"));
    assert!(rows[0].is_queued);

    let event = event_rx.try_recv().expect("authored PR update event");
    assert_eq!(event.event_name, "authored-prs-updated");
