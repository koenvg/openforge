...
- Plugins are trusted OpenForge packages declared in `package.json#openforge` with `id`, `apiVersion`, display metadata, optional `frontend` / `backend` built entry points, optional `frontendStyles` CSS artifact paths, and `requires` capabilities.
...
2. Define `package.json#openforge` with a unique plugin id, `apiVersion`, display metadata, optional `frontend` / `backend` built entry points, optional `frontendStyles`, and `requires` capabilities.
...
6. Build every plugin artifact referenced by `package.json#openforge`; OpenForge installs built entries and stylesheets and does not compile plugin source on install. Svelte/Vite builds that emit CSS must list each output in `frontendStyles`.
