            ...
            if let Some(ref session) = latest_session {
                ...
                if let Some(hooks_path) = hooks_path {
                    ...
                    let prompt = if resume_id.is_some() {
                        ...
                        drop(db_lock);
                        match task {
                            Some(ref t) => commands::orchestration::build_task_prompt(
                                t,
                    ...
                    match pty_mgr.spawn_claude_pty(
                        ...
                        Ok(_) => {
                            ...
                            println!(
                                "[startup] Claude Code task {} resumed (--resume: {})",
                                worktree.task_id, resume_id.is_some()
                            );
                            continue;
                        }
                        Err(e) => {
                ...
                // Spawn failed or no hooks path — mark interrupted
                let db = app.state::<Arc<Mutex<db::Database>>>();
                let db_lock = db.lock().unwrap();
