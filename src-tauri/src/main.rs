...
fn run_electron_sidecar() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ghostty_terminal_diagnostics_enabled = database
        .get_config(pty_manager::GHOSTTY_TERMINAL_DIAGNOSTICS_CONFIG)
        .ok()
        .flatten()
    ...
    pty_manager.set_terminal_diagnostics_enabled(ghostty_terminal_diagnostics_enabled);
