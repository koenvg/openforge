//! Bounded, cancellable supervision for macOS Keychain helper child processes.

use super::super::SecretStoreCancellation;
use std::{
    io::{self, Read, Write},
    os::fd::AsRawFd,
...
pub(super) const KEYCHAIN_PROCESS_POLL_INTERVAL: Duration = Duration::from_millis(10);
const MAX_KEYCHAIN_OUTPUT_BYTES: usize = 1024 * 1024;

#[derive(Debug)]
pub(super) enum WriteCommandError {
    NotCommitted(String),
    CommitUnknown(String),
}

pub(super) fn run_read_command(
    command: &mut Command,
    timeout: Duration,
    cancellation: &SecretStoreCancellation,
) -> Result<std::process::Output, String> {
    run_read_command_observing(command, timeout, cancellation, |_| {})
}

fn run_read_command_observing(
    command: &mut Command,
    timeout: Duration,
    cancellation: &SecretStoreCancellation,
    ...
    let mut stdout_pipe = match child.stdout.take() {
        Some(pipe) => pipe,
        None => {
            let cleanup_suffix = cleanup_suffix(terminate_and_reap(&mut child));
            return Err(format!(
    ...
    loop {
        ...
        let stdout_closed = match read_available_stdout(&mut stdout_pipe, &mut stdout, deadline) {
            Ok(StdoutRead::Closed) => true,
            Ok(StdoutRead::Pending) => false,
            Ok(StdoutRead::DeadlineExceeded) => {
                let cleanup_suffix = cleanup_suffix(terminate_and_reap(&mut child));
                return Err(format!(
...
pub(super) fn run_write_command(
    command: &mut Command,
    value: &[u8],
    timeout: Duration,
    cancellation: &SecretStoreCancellation,
) -> Result<ExitStatus, WriteCommandError> {
    let mut child = command
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .map_err(|error| {
            WriteCommandError::NotCommitted(format!(
                "Failed to start macOS Keychain writer: {error}"
            ))
        })?;
    let Some(mut stdin) = child.stdin.take() else {
        let cleanup_suffix = cleanup_suffix(terminate_and_reap(&mut child));
        return Err(WriteCommandError::NotCommitted(format!(
            "macOS Keychain writer input was unavailable{cleanup_suffix}"
        )));
    };
    if let Err(error) = stdin.write_all(value) {
        drop(stdin);
        let cleanup_suffix = cleanup_suffix(terminate_and_reap(&mut child));
        return Err(WriteCommandError::CommitUnknown(format!(
...
fn read_available_stdout(
    ...
    stdout: &mut Vec<u8>,
    deadline: Instant,
) -> Result<StdoutRead, String> {
    read_available_stdout_with_clock(stdout_pipe, stdout, deadline, Instant::now)
}

fn read_available_stdout_with_clock(
    ...
    loop {
        if now() >= deadline {
            return Ok(StdoutRead::DeadlineExceeded);
        }
        match reader.read(&mut buffer) {
            Ok(0) => return Ok(StdoutRead::Closed),
            Ok(read) => {
...
fn cleanup_suffix(cleanup: Result<(), String>) -> String {
    cleanup
        .err()
...
fn run_command_with_timeout_observing(
    ...
    command.args(args);
    run_read_command_observing(&mut command, timeout, cancellation, on_spawn)
}
...
mod tests {
    ...
    fn keychain_stdout_drain_stops_at_deadline_while_data_remains_readable() {
        ...
        let state = read_available_stdout_with_clock(&mut reader, &mut stdout, deadline, || {
            clock_checks += 1;
            if clock_checks == 1 {
                started_at
            } else {
        ...
        assert!(matches!(state, StdoutRead::DeadlineExceeded));
