...
fn accepted_backpressured_input_keeps_exact_suffix_and_original_unknown_receipt() {
    ...
    assert_eq!(
        client.write("accepted", &session.pty, 1, &input),
        Err(Error::OutcomeUnknown),
        "fixture must actually backpressure the accepted write"
    );
    ...
    assert!(matches!(
        fresh.write("sentinel", &session.pty, 2, sentinel),
        Ok(()) | Err(Error::OutcomeUnknown)
    ));
    wait_text(&fresh, &session.pty, "INPUT_DONE");
    let expected = [input.as_slice(), sentinel].concat();
    assert!(
        std::fs::read(received).unwrap() == expected,
        "accepted input was lost, reordered or duplicated"
    );
    assert_eq!(
        fresh.write("accepted", &session.pty, 1, &input),
        Err(Error::OutcomeUnknown)
