...
use std::os::{fd::{FromRawFd, RawFd}, unix::fs::MetadataExt};
...
impl Descriptor {
    fn capture(fd: RawFd) -> Result<Self, Error> {
        ...
        if flags < 0 || flags & libc::O_NONBLOCK == 0 || metadata.mode() & (libc::S_IFMT as u32) != libc::S_IFCHR as u32 {
        ...
        if unsafe { libc::ioctl(fd, libc::TIOCGWINSZ, &mut size) } < 0 { return Err(error(std::io::Error::last_os_error())); }
        ...
        Ok(Self { fd, device: metadata.dev(), inode: metadata.ino(), special_device: metadata.rdev() })
    }
}
...
impl Master {
    pub fn checkpoint(&self) -> Result<Descriptor, Error> {
        if !cfg!(all(target_os = "macos", target_arch = "aarch64")) { return Err(Error::UnsupportedReplacement); }
        let fd = match self {
    ...
    pub fn restore(descriptor: &Descriptor) -> Result<Self, Error> {
        if !cfg!(all(target_os = "macos", target_arch = "aarch64")) { return Err(Error::UnsupportedReplacement); }
        if Descriptor::capture(descriptor.fd)? != *descriptor { return Err(Error::InvalidRequest); }
        Ok(Self::Inherited { fd: descriptor.fd, active: false })
    }
    /// # Safety
    /// The previous image's owning wrapper must no longer exist; activate exactly once.
    pub unsafe fn activate(&mut self) {
        if let Self::Inherited { active, .. } = self { *active = true; }
    }
    pub fn resize(&self, size: PtySize) -> Result<(), Error> {
        match self {
            Self::Spawned(master) => master.resize(size).map_err(error),
            Self::Inherited { fd, .. } => {
                let size = libc::winsize { ws_row: size.rows, ws_col: size.cols, ws_xpixel: size.pixel_width, ws_ypixel: size.pixel_height };
                // SAFETY: size is a valid winsize and this wrapper retains fd until teardown.
                if unsafe { libc::ioctl(*fd, libc::TIOCSWINSZ, &size) } < 0 { Err(error(std::io::Error::last_os_error())) } else { Ok(()) }
            }
        }
    }
}
impl Drop for Master {
    fn drop(&mut self) {
        if let Self::Inherited { fd, active: true } = self {
            ...
            if unsafe { libc::close(*fd) } < 0 { eprintln!("retained PTY descriptor cleanup failed"); }
        }
    }
}
...
pub(crate) fn duplicate(fd: RawFd) -> Result<std::fs::File, Error> {
    ...
    if duplicate < 0 { return Err(error(std::io::Error::last_os_error())); }
...
impl ChildHandle {
    pub fn try_wait(&mut self) -> Result<Option<u32>, Error> {
        match self {
            Self::Spawned(child) => child.try_wait().map(|status| status.map(|status| status.exit_code())).map_err(error),
            Self::Inherited { pid, exit } => {
                if exit.is_none() { *exit = wait(*pid, libc::WNOHANG)?; }
                Ok(*exit)
            }
        }
    }
    pub fn wait(&mut self) -> Result<u32, Error> {
        match self {
            ...
            Self::Inherited { pid, exit } => {
                if exit.is_none() { *exit = wait(*pid, 0)?; }
...
fn wait(pid: i32, flags: i32) -> Result<Option<u32>, Error> {
    if pid <= 1 { return Err(Error::InvalidRequest); }
    loop {
        let mut status = 0;
        ...
        if result < 0 {
            ...
            if failure.kind() == std::io::ErrorKind::Interrupted { continue; }
            return Err(error(failure));
        }
        if !libc::WIFEXITED(status) && !libc::WIFSIGNALED(status) { return Err(Error::OutcomeUnknown); }
        use std::os::unix::process::ExitStatusExt;
        let status = portable_pty::ExitStatus::from(std::process::ExitStatus::from_raw(status));
        return Ok(Some(status.exit_code()));
    }
}
fn error(error: impl std::fmt::Display) -> Error { Error::Host(error.to_string()) }
...
mod tests {
    ...
    fn live_winsize_changes_are_not_descriptor_identity_changes() {
        ...
        pair.master.resize(PtySize { rows: 25, cols: 90, pixel_width: 0, pixel_height: 0 }).unwrap();
        assert!(Master::restore(&saved).is_ok(), "a live child may resize its tty while the owner is quiesced");
    ...
    fn retained_masters_reject_cross_binding_and_survive_last_slave_close() {
        ...
        assert!(Master::restore(&crossed).is_err(), "different PTYs must not share an identity");
        drop(first.slave);
        assert!(Master::restore(&original).is_ok(), "a child exit must not invalidate the retained master");
    }
    ...
    fn inherited_child_keeps_the_original_signal_exit_representation() {
        let mut child = std::process::Command::new("/bin/sh").args(["-c", "kill -TERM $$"]).spawn().unwrap();
        let mut retained = ChildHandle::Inherited { pid: child.id() as i32, exit: None };
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
        let code = loop {
            if let Some(code) = retained.try_wait().unwrap() { break code; }
        ...
        let expected = portable_pty::ExitStatus::from(std::process::ExitStatus::from_raw(libc::SIGTERM)).exit_code();
