...
unsafe extern "C" {
    fn csops(pid: libc::pid_t, operations: libc::c_uint, address: *mut libc::c_void, size: libc::size_t) -> libc::c_int;
}
pub(super) fn current() -> Result<String, Error> {
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    {
        ...
        if unsafe { csops(0, CS_OPS_CDHASH, hash.as_mut_ptr().cast(), hash.len()) } != 0 { return Err(Error::UnsupportedReplacement); }
        let identity = hash.iter().map(|byte| format!("{byte:02x}")).collect::<String>();
