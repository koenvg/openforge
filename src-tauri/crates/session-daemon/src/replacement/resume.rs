...
use std::{io::{Seek, SeekFrom}, os::{fd::AsRawFd, unix::process::CommandExt}};

struct Ready { runtime: RuntimeDirectory, resources: Resources, host: Host, manager: Manager }
pub(super) fn run(fd: i32, recovery: bool) -> Result<(), Error> {
    ...
    match initialize(&header, &bytes, start, recovery) {
        Ok(ready) => {
            ...
            unsafe { header.roots.release_primaries(); }
            drop(file);
            drop(bytes);
            ready.manager.clean();
        ...
        Err(_) if !recovery => {
            ...
            let _error = std::process::Command::new(&header.recovery.path).arg("--resume")
                .arg(file.as_raw_fd().to_string()).arg("recovery").env_clear().exec();
            Err(fatal())
        }
        Err(_) => Err(fatal()),
    }
}
fn initialize(header: &checkpoint::Header, bytes: &[u8], start: usize, recovery: bool) -> Result<Ready, Error> {
    let expected = if recovery { &header.recovery } else { &header.target };
    if version::current()? != expected.version || images::digest(&std::env::current_exe().map_err(|_| Error::Version)?)? != expected.sha256 { eprintln!("replacement initialization image identity mismatch"); return Err(Error::Version); }
    ...
    let runtime = stage("runtime metadata", RuntimeDirectory::reopen(&header.root, &header.credentials))?;
    let resources = stage("retained listeners and lock", header.roots.restore(runtime.path(), &runtime.socket_path(), header.agent_runtime.port))?;
    let mut host = stage("host resources", Host::restore(body.host, header.credentials.installation.clone(), header.agent_runtime.clone()))?;
    #[cfg(feature = "replacement-fixtures")]
    if !recovery && IMAGE_VERSION.ends_with("/openforge-session-daemon-fixture-init-fails") { return Err(Error::RecoveryUnavailable); }
    let manager = stage("replacement ledger", Manager::restored(body.manager, &header.operation, recovery, runtime.path().join("images")))?;
    let notifications = stage("notification journal reopen", crate::notification_journal::NotificationJournal::open(&runtime.path().join("notifications.sqlite")))?;
    stage("notification journal identity", header.notifications.validate())?;
    stage("gateway readiness", crate::server::start_gateway(&resources, &host, notifications))?;
    ...
    unsafe { host.activate_restored()?; }
    Ok(Ready { runtime, resources, host, manager })
}
