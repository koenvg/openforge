...
impl Descriptor {
    ...
    fn validate(&self) -> Result<(), Error> {
        if Self::capture(self.fd)? != *self {
            eprintln!(
                "replacement initialization: descriptor {} identity changed",
                self.fd
            );
...
impl Roots {
    pub fn capture(resources: &Resources) -> Result<Self, Error> {
        Ok(Self {
            ownership: Descriptor::capture(resources.ownership.as_raw_fd())?,
            control: Descriptor::capture(resources.control.as_raw_fd())?,
            agent: Descriptor::capture(resources.agent.as_raw_fd())?,
        })
    }
    pub fn descriptors(&self) -> [i32; 3] {
        [self.ownership.fd, self.control.fd, self.agent.fd]
    }
    pub fn restore(&self, runtime: &Path, socket: &Path, port: u16) -> Result<Resources, Error> {
        for descriptor in [&self.ownership, &self.control, &self.agent] {
            descriptor.validate()?;
        }
        let metadata = std::fs::symlink_metadata(runtime.join("daemon.lock"))
            .map_err(|_| Error::RecoveryUnavailable)?;
        if !metadata.is_file()
            || metadata.nlink() != 1
            || metadata.dev() != self.ownership.device
            || metadata.ino() != self.ownership.inode
        {
            eprintln!("replacement initialization: ownership path changed");
            return Err(Error::RecoveryUnavailable);
        }
        ...
        ownership.try_lock().map_err(|_| {
            eprintln!("replacement initialization: retained ownership lock unavailable");
        ...
        if control
            ...
            .as_pathname()
            != Some(socket)
            || agent.local_addr().map_err(|_| Error::RecoveryUnavailable)?
                != std::net::SocketAddr::from((std::net::Ipv4Addr::LOCALHOST, port))
        {
            eprintln!("replacement initialization: retained discovery address changed");
            return Err(Error::RecoveryUnavailable);
        }
        control
...
fn unconnected_stream(fd: i32) -> Result<(), Error> {
    ...
    if result == 0 || std::io::Error::last_os_error().raw_os_error() != Some(libc::ENOTCONN) {
        return Err(Error::RecoveryUnavailable);
    }
    Ok(())
}
