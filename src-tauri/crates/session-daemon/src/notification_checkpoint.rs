...
impl NotificationCheckpoint {
    pub fn capture(path: &Path) -> Result<Self, Error> {
        ...
        let (journal_id, acknowledged): (String, i64) = connection
            .query_row("SELECT id, acknowledged FROM journal", [], |row| {
                Ok((row.get(0)?, row.get(1)?))
            })
            .map_err(|_| Error::RecoveryUnavailable)?;
        let position: i64 = connection
            .query_row(
                "SELECT COALESCE((SELECT seq FROM sqlite_sequence WHERE name='notifications'), 0)",
                [],
                |row| row.get(0),
            )
            .map_err(|_| Error::RecoveryUnavailable)?;
        ...
        if uuid::Uuid::parse_str(&journal_id).is_err() || acknowledged > position {
            return Err(Error::RecoveryUnavailable);
        }
        let mut statement = connection
            .prepare("SELECT delivery FROM notifications ORDER BY position")
            .map_err(|_| Error::RecoveryUnavailable)?;
        ...
        while let Some(row) = rows.next().map_err(|_| Error::RecoveryUnavailable)? {
            ...
            if count > 4096 || total > 32 * 1024 * 1024 {
                return Err(Error::Capacity);
            }
            let delivery: NotificationDelivery =
                serde_json::from_str(&encoded).map_err(|_| Error::Version)?;
            delivery.envelope.validate()?;
            if delivery.journal_id != journal_id
                || delivery.position == 0
