...
impl InputCheckpoint {
    pub fn retained_bytes(&self) -> usize {
        self.pending.iter().map(|pending| pending.bytes.len()).fold(0, usize::saturating_add)
    }
    pub fn validate(&self) -> Result<(), Error> {
        if self.format != 1 { return Err(Error::Version); }
        if self.failed { return Err(Error::RecoveryUnavailable); }
        if self.pending.len() > MAX_WRITES { return Err(Error::Capacity); }
        let mut total = 0usize;
        for pending in &self.pending {
            if pending.offset >= pending.bytes.len() { return Err(Error::InvalidRequest); }
            total = total.checked_add(pending.bytes.len()).ok_or(Error::Capacity)?;
            if total > MAX_BYTES { return Err(Error::Capacity); }
        }
        Ok(())
    }
}
...
impl InputWriter {
    pub fn new(writer: Box<dyn Write + Send>) -> Self {
        Self { writer, state: InputCheckpoint { format: 1, pending: VecDeque::new(), failed: false } }
    }
    ...
    pub fn submit(&mut self, bytes: &[u8]) -> Result<(), Error> {
        if self.state.failed { return Err(Error::OutcomeUnknown); }
        let retained: usize = self.state.pending.iter().map(|pending| pending.bytes.len()).sum();
        if retained.saturating_add(bytes.len()) > MAX_BYTES || self.state.pending.len() >= MAX_WRITES {
            return Err(Error::Capacity);
        }
        if !bytes.is_empty() {
            self.state.pending.push_back(Pending { bytes: bytes.to_vec(), offset: 0 });
        }
        ...
        while let Some(pending) = self.state.pending.front_mut() {
            match self.writer.write(&pending.bytes[pending.offset..]) {
                ...
                Err(error) if matches!(error.kind(), std::io::ErrorKind::WouldBlock | std::io::ErrorKind::Interrupted) => {
                    if Instant::now() >= deadline { return Err(Error::OutcomeUnknown); }
...
mod tests {
    ...
    fn partial_accepted_input_keeps_its_offset_and_order_through_checkpoint_restore() {
        ...
        let mut writer = InputWriter::new(Box::new(Sink { bytes: Arc::clone(&bytes), allowance: 2 }));
        ...
        drop(writer);
        let mut resumed = InputWriter::restore(Box::new(Sink {
            bytes: Arc::clone(&bytes), allowance: usize::MAX,
        }), saved).unwrap();
