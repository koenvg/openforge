...
use crate::Client;
use openforge_session_protocol::{Capabilities, Command, Error, OperationId, ReplacementPhase, ReplacementState, ReplacementStatus, Response};
...
impl Client {
    ...
    pub fn replacement_phase(&self, operation: OperationId, phase: ReplacementPhase) -> Result<ReplacementStatus, Error> {
        match self.request(Command::Replacement { controller: self.controller.clone(), operation, phase })? {
    ...
    pub fn replacement_status(&self, operation: &OperationId) -> Result<ReplacementStatus, Error> {
        match self.request(Command::ReplacementStatus { operation: operation.clone() })? {
            Response::Replacement(value) => Ok(value),
            _ => Err(Error::OutcomeUnknown),
        }
    }
    pub(crate) fn complete_replacement(&self, operation: OperationId, phase: ReplacementPhase) -> Result<(), Error> {
        let deadline = Instant::now() + Duration::from_secs(60);
        let mut response = self.replacement_phase(operation.clone(), phase.clone());
        loop {
            match response {
                Ok(status) => {
                    if status.operation != operation { return Err(Error::OutcomeUnknown); }
                    match (&phase, &status.state) {
                        ...
                        (_, ReplacementState::Failed { stage }) => return Err(Error::Host(format!("replacement failed during {stage:?}; activation was unsuccessful"))),
                        (_, ReplacementState::Aborted | ReplacementState::Activated) => return Err(Error::OperationConflict),
                        _ => {}
                    }
                }
