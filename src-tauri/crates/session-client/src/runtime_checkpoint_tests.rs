...
fn reopening_owned_runtime_never_creates_rotates_or_reclaims_credentials() {
    ...
    let changed = Credentials { installation: runtime.credentials().installation.clone(), token: "f".repeat(64) };
    fs::write(&path, serde_json::to_vec(&changed).unwrap()).unwrap();
    assert!(RuntimeDirectory::reopen(root.path(), runtime.credentials()).is_err());
    assert_eq!(fs::read(&path).unwrap(), serde_json::to_vec(&changed).unwrap());
