...
impl RuntimeDirectory {
    ...
    pub fn reopen(root: &Path, expected: &Credentials) -> Result<Self, Error> {
        ...
        if !metadata.is_dir() || metadata.uid() != uid() || metadata.mode() & 0o022 != 0 { return Err(Error::Unauthorized); }
        ...
        let file = OpenOptions::new().read(true).custom_flags(libc::O_NOFOLLOW | libc::O_NONBLOCK)
            .open(&credential_path).map_err(io_error)?;
        let metadata = file.metadata().map_err(io_error)?;
        if !metadata.is_file() || metadata.uid() != uid() || metadata.mode() & 0o077 != 0 || metadata.len() > 4096 { return Err(Error::Unauthorized); }
        let mut bytes = Vec::new();
        file.take(4097).read_to_end(&mut bytes).map_err(io_error)?;
        if bytes.len() > 4096 { return Err(Error::Capacity); }
        let credentials: Credentials = serde_json::from_slice(&bytes).map_err(|_| Error::Unauthorized)?;
        if uuid::Uuid::parse_str(credentials.installation.as_str()).is_err()
            || credentials.token.len() != 64 || !credentials.token.bytes().all(|byte| byte.is_ascii_hexdigit())
            || credentials.installation != expected.installation || credentials.token != expected.token
