...
The hook endpoint will derive the scoped session, plugin owner, PTY instance, and policy identity from the host-issued scoped credential and environment. It will reject caller-supplied ownership and stale PTY instances. The walkthrough coordinator will bind the active attempt to the turn id observed after Generate and will treat the matching `Stop` transition as normal attempt completion. A later terminal follow-up gets another turn id and cannot finish an older attempt.
