...
Run terminal presentation and Markdown visuals in an ARM64 Playwright Ubuntu Noble container pinned by immutable digest and aligned with the lockfile's Playwright version. Run that container on GitHub's `ubuntu-24.04-arm` image and expose the same update and check commands to Apple Silicon developers. Install dependencies and build the same sources inside the container. This avoids tying approved pixels to the mutable host runner image while keeping baseline maintenance practical on the project's primary local architecture. Approved terminal baselines move from `darwin-arm64` to `linux-arm64`; Markdown baselines are regenerated in the same container after removing their Darwin-only execution guard. Review every changed image before accepting it. Do not widen tolerances to make the migration pass.

Using a plain `ubuntu-latest` host was rejected because image updates can alter browser libraries and fonts underneath approved baselines. An x64 container was rejected because Chromium is not reliable across browser contexts under Rancher Desktop's Apple Silicon emulation, so developers could not reproduce the canonical update command locally. Reusing macOS only for screenshots was also rejected because it would preserve one of the longest routine macOS jobs after its only native assertion moved back to the Rust suite.
