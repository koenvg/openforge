...
- Move terminal and Markdown browser visual conformance to a pinned Linux environment with reviewed Linux baselines. Keep its native PTY assertion covered by the macOS Rust suite instead of running the same Rust test again inside the visual job.
