...
**Reason**: Scoped Agent Sessions now use each configured provider's normal interactive authentication, account selection, and onboarding behavior instead of a host authentication preflight.

**Migration**: Remove the scoped authentication error path and display the provider-native flow in the mounted terminal.
