...
- [ ] 8.2 Add the verified updater helper and integrate source installation with the restart protocol; verify isolated install tests preserve active sessions, restore the old bundle on supported pre-commit replacement failures, and never use blanket process-name kills for daemon cleanup.
  - KVG-5206 checkpoint: packaging now includes the authenticated native helper. Isolated real-process tests cover preparation, cancellation, host-exit fencing, replacement, authorized launch roots and durable commit. Production driver/source installer wiring, daemon activation, first-adoption approval and packaged continuity remain incomplete. See `docs/update-helper-transaction.md`; this task stays unchecked.
