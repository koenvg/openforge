...
- [ ] 8.2 Add the verified updater helper and integrate source installation with the restart protocol; verify isolated install tests preserve active sessions, restore the old bundle on supported pre-commit replacement failures, and never use blanket process-name kills for daemon cleanup.
  - KVG-5206 checkpoint: packaging includes the authenticated native helper. The coordinator requires owned Sidecar exit after detach; separate first-adoption consent binds installed and target bytes. Runtime maintenance pins matching assets, activates the daemon and checks fresh-controller identity before commit. Native launch records kernel process identity. A registered Sidecar verifies inherited admission and its loaded kernel CDHash against authorized executable bytes before domain access. The native contract rejects a distinct running image even after authorized files are restored; a separate Rust test preserves a real PTY. Latest root, native/helper/Sidecar, daemon all-feature and packaging runs pass; earlier failures remain documented in `docs/update-helper-transaction.md`. Production driver/source installer and startup wiring, Electron image/readiness and independent commit checks, legacy shutdown and packaged continuity remain incomplete. This task stays unchecked.
