...
- [ ] 8.2 Add the verified updater helper and integrate source installation with the restart protocol; verify isolated install tests preserve active sessions, restore the old bundle on supported pre-commit replacement failures, and never use blanket process-name kills for daemon cleanup.
  - KVG-5206 disabled checkpoint: the packaged helper, local driver, production startup/admission/readiness callbacks, atomic exchange, durable launch gate and native-first commit are implemented. Restored windows acknowledge each replacement lifetime. Source parent-loss protection is armed before domain startup, but original source birth/exit attestation and preparation/Installed recovery remain unfinished. Current native contracts, helper/backend suites, rebuilt arm64 package sealing and isolated ordinary-startup smoke pass. These are not full packaged update/agent/tool continuity evidence. The menu, source installer, published path and legacy adoption remain disabled. KVG-4730 retains packaged acceptance; KVG-1789 retains publication gates. See `docs/update-helper-transaction.md`. This task stays unchecked.
...
- [x] 9.3 Run full desktop checks using `pnpm test`, `pnpm exec tsc --noEmit`, `pnpm lint`, `pnpm electron:contract:check`, and `pnpm mobile:contract:check`; verify passing output or document exact blockers. Run additional Companion app checks if its Dart code changes.
  - Disabled checkpoint validation after rebase onto `b1a6a0e7d`: 896 root test files passed, 7,678 tests passed, 3 expected failures and 96 skips. TypeScript, plugin-host types, lint and both contract checks passed. Logs: `/tmp/KVG-5206-disabled-*.log`. No Dart implementation changed; this does not complete packaged-update acceptance.
