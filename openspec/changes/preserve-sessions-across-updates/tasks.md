...
- [ ] 8.2 Add the verified updater helper and integrate source installation with the restart protocol; verify isolated install tests preserve active sessions, restore the old bundle on supported pre-commit replacement failures, and never use blanket process-name kills for daemon cleanup.
  - KVG-5206 checkpoint: packaging includes the authenticated native helper. The coordinator requires owned Sidecar exit after detach; separate first-adoption consent binds installed and target bytes. Runtime maintenance pins matching assets, activates the daemon and checks fresh-controller identity before commit. Native launch records kernel process identity; Electron and admitted Sidecar processes must match their authorized executable CDHashes. A private Electron runtime exercises startup and raw-ASAR staging/measurement, but uses a fixture main module and Sidecar, not production boot. Separate Rust tests preserve a real PTY. Latest root, native/helper/backend and package build checks pass; strict signature verification of the packaged Electron executable still fails its resource seal, so local integrity sealing remains required. Transient backend openpty failures passed focused/full reruns and are tracked separately in KVG-5245. Earlier daemon evidence remains documented in `docs/update-helper-transaction.md`. Production driver/source installer and startup wiring, cold readiness and independent commit checks, legacy shutdown and packaged continuity remain incomplete. This task stays unchecked.
