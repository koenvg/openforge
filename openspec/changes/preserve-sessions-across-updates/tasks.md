...
- [ ] 8.2 Add the verified updater helper and integrate source installation with the restart protocol; verify isolated install tests preserve active sessions, restore the old bundle on supported pre-commit replacement failures, and never use blanket process-name kills for daemon cleanup.
  - KVG-5206 checkpoint: packaging includes the authenticated native helper. The coordinator requires owned Sidecar exit after detach. Separate first-adoption consent binds installed and target bytes. The helper now borrows controller authority without fencing Sidecar, pins matching runtime/CLI assets, activates a distinct daemon image and checks fresh-controller runtime identity before commit. The 12-test native contract preserves daemon PID/lifetime; a separate Rust test preserves a real PTY. Production driver/source installer wiring, authenticated target startup, legacy shutdown and packaged continuity remain incomplete. Latest broad checks have timeout failures documented in `docs/update-helper-transaction.md`; this task stays unchecked.
