...
A shared host admission controller allows two in-flight document reads across all adapters, with no waiting queue. Excess calls reject as busy and offer manual retry. The slot covers reading, encoding, and response construction. Apply a 15-second response deadline and discard late results. On timeout or host request cancellation, signal the reader to stop between filesystem operations, but retain its handles and admission slot until the underlying I/O actually exits. Blocking filesystem operations cannot be forcibly cancelled; a stalled mount may occupy capacity beyond the response deadline. Never release capacity early and admit additional actual reads. Close handles and release the slot when that work exits on every success, failure, or cancellation path. This response-only deadline was explicitly approved for KVG-5075. The caller never loops retries automatically.
