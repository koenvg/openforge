...
export async function exchangeDaemon(runtime, credentials, command) {
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('connect', () => {
      const body = Buffer.from(JSON.stringify({ version: 4, body: { token: credentials.token, command } }))
    ...
    socket.on('data', chunk => {
      ...
      if (reply?.version !== 4) return finish(new Error('Invalid daemon fixture reply version'))
      if (!reply.body?.Ok) {
