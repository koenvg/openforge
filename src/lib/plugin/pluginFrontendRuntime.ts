...
function createUnavailableFrontendApi(pluginId: string): FrontendOpenForgeAPI {
  ...
  return {
    ...
    commands: {
      ...
      listInstalledProviders: unavailable('commands.listInstalledProviders'),
    },
    events: {
    ...
    fs: {
      ...
      task: {
        ...
        readDocument: async () => { throw new Error('DOCUMENT_PREVIEW_UNAVAILABLE_HOST: task documents are unavailable') },
