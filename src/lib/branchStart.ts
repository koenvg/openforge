...
import type { DivergenceResolution, Task } from './types'
 ...
 */
export async function resolveBranchStart(task: Task, repoPath: string): Promise<BranchStartOutcome> {
  const branch = task.worktree_branch?.trim() ?? ''
  if (task.worktree_source !== 'existingBranch' || branch === '') {
