...
it('maps task document requests to the typed camelCase host command', async () => {
  const result = { status: 'unavailable', reason: 'too-large', size: 16777217, maxBytes: 16777216 }
  invoke.mockResolvedValueOnce(result)
  await expect(ipc.taskFsReadDocument('T-1', 'docs/a.pdf')).resolves.toEqual(result)
  expect(invoke).toHaveBeenCalledWith('task_fs_read_document', { taskId: 'T-1', filePath: 'docs/a.pdf' })
})
