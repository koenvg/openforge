...
describe('Electron plugin:// protocol security contract', () => {
  ...
  it('keeps renderer CSP compatible with plugin:// import maps and sidecar IPC without unsafe filesystem access', () => {
    ...
    expect(ELECTRON_RENDERER_CSP).toContain(`connect-src 'self' plugin: http://127.0.0.1:${DEFAULT_SIDECAR_PORT} https://api.github.com https://*.atlassian.net`)
    expect(cspDirective(ELECTRON_RENDERER_CSP, 'worker-src')).toBe("worker-src 'self' blob:")
