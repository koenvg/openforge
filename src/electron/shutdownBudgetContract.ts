 ...
 * 1. Electron stops the sidecar event stream and waits at most
 *    `SIDECAR_EVENT_STREAM_TEARDOWN_TIMEOUT_MS` before signaling the process.
 ...
 *
 * Invariant: event stream teardown + SIGTERM grace < coordinator deadline,
 * and Rust cleanup timeout < SIGTERM grace. This leaves room for Rust cleanup
 * to finish before Electron escalates, and room for stopSidecar to return a
 * structured SIGKILL report before the coordinator reports a quit-time cleanup
 * timeout.
 */
export const SIDECAR_EVENT_STREAM_TEARDOWN_TIMEOUT_MS = 250
...
export interface RustSidecarShutdownBudgetContract {
  readonly eventStreamTeardownTimeoutMs: number
...
export const RUST_SIDECAR_SHUTDOWN_BUDGET_CONTRACT: RustSidecarShutdownBudgetContract = Object.freeze({
  eventStreamTeardownTimeoutMs: SIDECAR_EVENT_STREAM_TEARDOWN_TIMEOUT_MS,
...
export function assertRustSidecarShutdownBudgetContract(
  ...
  if (!(contract.eventStreamTeardownTimeoutMs + contract.sigtermGraceMs < contract.coordinatorDeadlineMs)) {
    throw new Error('Electron event stream teardown plus SIGTERM grace must be less than the shutdown coordinator deadline')
  }
}
