...
describe('FileQuickOpen', () => {
  ...
  it('searches a single nonblank character but not whitespace, retaining the original query', async () => {
    vi.useFakeTimers({ toFake: ['setTimeout', 'clearTimeout'] })
    renderFileQuickOpen()
    const input = getDialogInput()
    ...
    await vi.advanceTimersByTimeAsync(150)
    expect(mockFsSearchFiles).not.toHaveBeenCalled()
    expect(screen.getByRole('status').textContent).toContain('Type to search files...')
    await fireEvent.input(input, { target: { value: ' a ' } })
    await vi.advanceTimersByTimeAsync(150)
    expect(mockFsSearchFiles).toHaveBeenCalledExactlyOnceWith('test-project-id', ' a ', 50)
  })

  it('announces loading without activating old results and shows the 50-file limit', async () => {
    vi.useFakeTimers({ toFake: ['setTimeout', 'clearTimeout'] })
    mockFsSearchFiles.mockResolvedValueOnce(Array.from({ length: 50 }, (_, i) => `src/file-${i}.ts`))
    renderFileQuickOpen()
    const input = getDialogInput()
    await fireEvent.input(input, { target: { value: 'file' } })
    await vi.advanceTimersByTimeAsync(150)
    expect(screen.getAllByRole('option')).toHaveLength(50)
    expect(screen.getByText('Showing top 50 results')).toBeTruthy()
    let resolveSearch!: (paths: string[]) => void
    mockFsSearchFiles.mockReturnValueOnce(new Promise(resolve => { resolveSearch = resolve }))
    await fireEvent.input(input, { target: { value: 'next' } })
    await vi.advanceTimersByTimeAsync(150)
    expect(screen.getByRole('status').textContent).toContain('Searching...')
    expect(input.getAttribute('aria-activedescendant')).toBeNull()
    await fireEvent.keyDown(input, { key: 'Enter' })
    expect(mockRevealFileInFileViewer).not.toHaveBeenCalled()
    resolveSearch(['src/next.ts'])
    await tick()
    await tick()
    expect(screen.queryByText('Showing top 50 results')).toBeNull()
    await fireEvent.click(screen.getByRole('option', { name: /next\.ts/ }))
    await tick()
    expect(mockRevealFileInFileViewer).toHaveBeenCalledWith('src/next.ts')
    expect(mockNavigate).toHaveBeenCalledWith(FILE_VIEWER_VIEW_KEY)
  })
})
