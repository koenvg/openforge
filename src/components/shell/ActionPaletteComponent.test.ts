...
describe('ActionPalette component', () => {
  ...
  it('shows CMD+K as the toggle hint', async () => {
    ...
    expect(screen.getByLabelText('Enter').parentElement?.textContent).toContain('execute')
    expect(screen.getByText('Esc').parentElement?.textContent).toContain('close')
  ...
  it('shows allowed merge methods and confirms the selected method before execution', async () => {
    ...
    expect(screen.getByLabelText('Enter').parentElement?.textContent).toContain('confirm')
    expect(screen.getByText('Esc').parentElement?.textContent).toContain('cancel')
  ...
  it('cancels confirmation back to the filtered search and preserves selection without executing', async () => {
    const { default: ActionPalette } = await import('./ActionPalette.svelte')
    const onExecute = vi.fn()
    const onClose = vi.fn()
    render(ActionPalette, { task: makeTask({ id: 'T-100', status: 'doing' }), taskPrs: [makePullRequest()], onExecute, onClose })
    const input = screen.getByRole('combobox')
    await fireEvent.input(input, { target: { value: 'rebase' } })
    await fireEvent.keyDown(input, { key: 'Enter' })
    const confirm = screen.getByRole('button', { name: 'Confirm' })
    await fireEvent.keyDown(confirm, { key: 'Enter', repeat: true })
    expect(onExecute).not.toHaveBeenCalled()
    await fireEvent.keyDown(confirm, { key: 'Escape' })
    const restored = screen.getByRole('combobox')
    expect((restored as HTMLInputElement).value).toBe('rebase')
    expect(document.activeElement).toBe(restored)
    expect(screen.getByRole('option', { selected: true }).textContent).toContain('Rebase')
