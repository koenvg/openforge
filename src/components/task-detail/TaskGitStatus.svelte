...
<CollapsibleSection sectionKey="git-status" title="Changes" cardId="git-status">
  ...
  {#if errored}
    <div class="py-2 text-xs text-of-text/50">Unable to read changes</div>
  {:else}
    <div class="flex flex-col gap-1.5 py-2">
      <div class="flex items-center justify-between gap-2">
        <span class="text-xs text-of-text/55">Remote</span>
        {#if !summary?.has_remote}
          <span class="text-xs text-of-text/40">no remote</span>
        {:else if summary.remote_ahead === 0 && summary.remote_behind === 0}
          <span class="text-xs text-of-text/50">up to date</span>
        {:else}
          <span class="flex items-center gap-1.5 font-mono text-xs" aria-label="Commits ahead and behind remote">
            {#if summary.remote_ahead > 0}<span class="text-of-success">↑{summary.remote_ahead}</span>{/if}
            {#if summary.remote_behind > 0}<span class="text-of-warning">↓{summary.remote_behind}</span>{/if}
          </span>
        {/if}
      </div>

      <div class="flex items-center justify-between gap-2">
        <span class="text-xs text-of-text/55">Commits</span>
        {#if (summary?.local_commits ?? 0) === 0}
          <span class="text-xs text-of-text/50">none</span>
        {:else}
          <span class="font-mono text-xs text-of-text/70" aria-label="Local commits">{summary.local_commits} {summary.local_commits === 1 ? 'commit' : 'commits'}</span>
        {/if}
      </div>

      <div class="flex items-center justify-between gap-2">
        <span class="text-xs text-of-text/55">Uncommitted</span>
        {#if (summary?.uncommitted_files ?? 0) === 0}
          <span class="text-xs text-of-text/50">none</span>
        {:else}
          <span class="flex items-center gap-2 font-mono text-xs" aria-label="Uncommitted changes">
            <span class="text-of-text/70">{summary.uncommitted_files} {summary.uncommitted_files === 1 ? 'file' : 'files'}</span>
            <span class="text-of-success">+{summary.insertions}</span>
            <span class="text-of-danger">−{summary.deletions}</span>
          </span>
        {/if}
      </div>

      <div class="flex items-center justify-between gap-2">
        <span class="text-xs text-of-text/55">Untracked</span>
        {#if (summary?.untracked_files ?? 0) === 0}
          <span class="text-xs text-of-text/50">none</span>
        {:else}
          <span class="flex items-center gap-2 font-mono text-xs" aria-label="Untracked files">
            <span class="text-of-text/70">{summary.untracked_files} {summary.untracked_files === 1 ? 'file' : 'files'}</span>
            <span class="text-of-success">+{summary.untracked_insertions}</span>
          </span>
        {/if}
      </div>
    </div>
